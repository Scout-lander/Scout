...
namespace HeathenEngineering.SteamworksIntegration.UI
{
    public class LobbyUIControl : MonoBehaviour
    {
        ...
        public event Action<UserData, bool> OnPlayerReadyStatusChanged; // Event to notify ready status changes
        ...
        private void Start()
        {
            ...
            if (API.App.Initialized)
            {
                ...
                // Check if player is solo in lobby
                if (Lobby != null && Lobby.Members.Length == 1)
                {
                    // Set the solo player to ready by default
                    IsPlayerReady = true;
                    Debug.Log("Solo player detected. Player set to ready by default.");
                }
            }
            else
            {
        ...
        public void SetLobbyGameReady()
        {
            API.Matchmaking.Client.SetLobbyData(Lobby, "GameReady", "true");
            Debug.Log("Game Starting");
        }

        public void StartGame()
        {
            if (allReady && IsPlayerOwner)
            {
                Debug.Log("All players are ready. Starting game...");
                // Load the game scene for all players
            //FishNet.Network.NetworkManager.Singleton.ServerManager.SceneManager.LoadScene("GameScene", FishNet.Scene.LoadOptions.Players);
            }
            else
            {
                Debug.LogWarning("Not all players are ready or you are not the host.");
            }
        }
        ...
        {
            IsPlayerReady = false;
            OnPlayerReadyStatusChanged?.Invoke(UserData.Me, false); // Trigger event with UserData.Me
            RefreshUI();
        }
        ...
        {
            IsPlayerReady = true;
            OnPlayerReadyStatusChanged?.Invoke(UserData.Me, true); // Trigger event with UserData.Me
            RefreshUI();
        }

        // Call this when the player toggles their ready state
    public void ToggleReadyStatus(UserData userData, bool isReady)
    {
        // Update ready status logic here
        OnPlayerReadyStatusChanged?.Invoke(userData, isReady); // Trigger the event
    }

    // Example function to handle ready button click
    public void OnReadyButtonClicked()
    {
        bool isReady = true; // Example toggle logic for ready
        ToggleReadyStatus(UserData.Me, isReady); // Notify that the player is ready
    }

    public void OnNotReadyButtonClicked()
    {
        bool isReady = false;
        ToggleReadyStatus(UserData.Me, isReady); // Notify that the player is not ready
    }
        ...
        public void RefreshUI()
        {
            ...
            {
                // Show the Start Game button only for the host and when all players are ready
                startGameButton.gameObject.SetActive(IsPlayerOwner && allReady);
                ...
                if (readyButton != null)
                    readyButton.gameObject.SetActive(!IsPlayerReady && Lobby.Members.Length > 1);

                if (notReadyButton != null)
                    notReadyButton.gameObject.SetActive(IsPlayerReady && Lobby.Members.Length > 1);
                ...
                {
                    var isMemberReady = API.Matchmaking.Client.GetLobbyMemberData(Lobby, member.user, LobbyData.DataReady) == "true";
                    if (!isMemberReady)
                    {
                        allReady = false;
                        break;
